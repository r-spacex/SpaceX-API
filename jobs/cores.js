...
export default async () => {
  try {
    ...
    const scrapedActive = [];
    $(
      'div.md:nth-child(2) > table:nth-child(15) > tbody:nth-child(2) > tr'
    ).each((index, element) => {
    ...
    const unknown = $(
      'body > div.content > div > div > table:nth-child(18) > tbody'
    ).text();
    ...
    const unknownStatus = unknownRow
      .filter((value, index) => (index + 1) % 6 === 0)
    ...
    const inactive = $(
      'body > div.content > div > div > table:nth-child(21) > tbody'
    ).text();
    ...
    const inactiveStatus = inactiveRow
      .filter((value, index) => (index + 1) % 6 === 0)
      .map((x) => x.replace(/\[source\]/gi, ''));
    ...
    const lost = $(
      'body > div.content > div > div > table:nth-child(25) > tbody'
    ).text();
    ...
    const reuseUpdates = cores.docs.map(async (core) => {
      if (!core?.id) return;
      const [rtlsAttempts, rtlsLandings, asdsAttempts, asdsLandings] =
        await Promise.all([
