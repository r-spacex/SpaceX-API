...
v2.get('/latest', cache.route(5000), asyncHandle(async (req, res) => {
...
v2.get('/', cache.route(5000), asyncHandle(async (req, res) => {
  const data = await fetchLaunch('launch_v2', req);
  res.json(data);
}));

v2.get('/all', cache.route(5000), asyncHandle(async (req, res) => {
