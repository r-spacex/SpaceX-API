...
if [ $TRAVIS_BRANCH == 'master' ]; then
    ...
    ssh -t -o StrictHostKeyChecking=no $SERVER_USER@$SERVER_IP "bash --login -c 'cd /var/www/spacex/SpaceX-API && git checkout master && git pull --force && yarn --production && pm2 reload spacex-api'"
else
