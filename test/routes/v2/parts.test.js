...
test('It should return all v2 capsules', async () => {
  const response = await request.get('/v2/parts/caps');
...
test('It should return capsule C101', async () => {
  const response = await request.get('/v2/parts/caps/C101');
  expect(response.statusCode).toBe(200);
...
test('It should return all v2 cores', async () => {
  const response = await request.get('/v2/parts/cores');
  expect(response.statusCode).toBe(200);
  response.body.forEach((item) => {
...
test('It should return core B0007', async () => {
  const response = await request.get('/v2/parts/cores/B0007');
  expect(response.statusCode).toBe(200);
