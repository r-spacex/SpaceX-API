...
test("It should return Dragon data", () => {
  return request(app).get("/v2/capsules").then(response => {
    ...
    response.body.forEach(item => {
      ...
      item.thrusters.forEach(item => {
        expect(item).toHaveProperty("type", expect.any(String))
        expect(item).toHaveProperty("amount", expect.any(Number))
        expect(item).toHaveProperty("pods", expect.any(Number))
        expect(item).toHaveProperty("fuel_1", expect.any(String))
        expect(item).toHaveProperty("fuel_2", expect.any(String))
        expect(item).toHaveProperty("thrust.kN", expect.any(Number))
        expect(item).toHaveProperty("thrust.lbf", expect.any(Number))
      })
